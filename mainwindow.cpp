...
void MainWindow::createSystemTrayIcon()
{
    ...
    trayIcon->setIcon(QIcon(_G(":/new/background/images/icon.ico")));
    trayIcon->setToolTip(_G("酷音听听 - 音乐播放器"));

    restoreAction=new QAction(_G("打开主面板"), this);
    connect(restoreAction, SIGNAL(triggered()), this, SLOT(show()));

    quitAction=new QAction(_G("退出"), this);
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    if(trayIcon->isVisible()) {
        hide();
        trayIcon->showMessage(_G("提示"), _G("酷音听听将在后台运行"));
        event->ignore();
    } else {
        event->accept();
    }
...
void MainWindow::updateSongList(int i)
{
    ...
    ui->label->setText(_G("正在播放: %1").arg(ui->tableWidget->item(i, 0)->text()));
}
...
void MainWindow::importSongs()
{
    ...
    QStringList pathList=QFileDialog::getOpenFileNames(this, _G("选择文件"), initialName, _G("*.mp3"));
...
{
    if(ui->toolButton_Play->text()==_G("播放")) {
        player->play();
        ui->toolButton_Play->setText(_G("暂停"));
    } else {
        player->pause();
        ui->toolButton_Play->setText(_G("播放"));
    }
}
...
{
    QMessageBox::about(this, _G("关于酷音听听"), \
                       _G("酷音听听，感谢您的使用！"));
}
